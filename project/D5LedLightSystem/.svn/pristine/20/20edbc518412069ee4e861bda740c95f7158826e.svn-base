//
//  D5LedLinkDeviceDeleteFromApp.m
//  D5LedLightSystem
//
...
#import "D5LedLinkDeviceDeleteFromApp.h"

@implementation D5LedLinkDeviceDeleteFromApp

- (void)ledLinkDeviceDeleteFromApp:(NSArray *)deleteInfos {
    @autoreleasepool {
        LedHeader *header = [self headerForCmd:Cmd_Link_Devices withSubCmd:SubCmd_Device_Delete_From_App];
        
        NSMutableData *body = [[NSMutableData alloc] init];
        if (deleteInfos.count > 0) {
            for (NSData *deleteInfo in deleteInfos) {
                [body appendData:deleteInfo];
            }
        }
...
- (void)getResult:(LedHeader *)header body:(NSDictionary *)body from:(NSString *)ip{
    @autoreleasepool {
        if (header->cmd != Cmd_Link_Devices || header->subCmd != SubCmd_Device_Delete_From_App) {
            return;
        }
        ...
        [[D5LedModule sharedLedModule].resultMuticast.delegateList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                if (obj && [obj respondsToSelector:@selector(ledLinkDeviceDeleteFromAppReturn:)]) {
                    [obj ledLinkDeviceDeleteFromAppReturn:status];
                }
            }
        }];
