...
#import "D5LedBoxCleanDeepProgress.h"

@implementation D5LedBoxCleanDeepProgress

- (void)ledBoxCleanDeepProgress {
    @autoreleasepool {
        LedHeader *header = [self headerForCmd:Cmd_Box_Operate withSubCmd:SubCmd_Box_Clean_Deep_Progress];
...
- (void)getResult:(LedHeader *)header body:(NSDictionary *)body from:(NSString *)ip{
    @autoreleasepool {
        if (header->cmd != Cmd_Box_Operate || header->subCmd != SubCmd_Box_Clean_Deep_Progress) {
            return;
        }
        ...
        [[D5LedModule sharedLedModule].resultMuticast.delegateList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                if (obj && [obj respondsToSelector:@selector(ledBoxCleanDeepProgressReturn:)]) {
                    [obj ledBoxCleanDeepProgressReturn:progress];
                }
            }
        }];
    }
