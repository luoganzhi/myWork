...
@interface D5LightGroupViewController () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, D5LightGroupNumberViewControllerDelegate, D5LedListDelegate>
...
- (void)wifiWeakTipViewIsHidden:(BOOL)isHidden {
    __weak typeof(self) weakSelf = self;
    if (isHidden) { // WiFi信号强，隐藏提示view
        if (self.wifiWeakTipLabel.hidden) {
            return ;
        }
        ...
        self.wifiWeakTipLabel.hidden = NO;
        self.wifiWeakTipLabel.alpha = 0.0f;
        [self.view layoutIfNeeded];
        [UIView animateWithDuration:0.2f animations:^{
            typeof(weakSelf) strongSelf = weakSelf;
...
- (void)startNotifierNetWork {
    ...
    GLobalRealReachability.wifiWeakBlock = ^ {
        typeof(weakSelf) strongSelf = weakSelf;
        [strongSelf wifiWeakTipViewIsHidden:NO];
    };
    
    GLobalRealReachability.wifiStrongBlock = ^ {
        typeof(weakSelf) strongSelf = weakSelf;
        [strongSelf wifiWeakTipViewIsHidden:YES];
    };
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self resignGuideTip];
    [self wifiWeakTipViewIsHidden:YES];
...
- (void)lightNumbered:(NumberButtonType)type meshAddr:(NSInteger)meshAddr selectedIndex:(NSInteger)index {
    ...
    switch (type) {
        case NumberButtonTypeSure: {
            ...
            if (ledData.lightId == index) { // 灯的编号和目标编号一致
                return;
            }
