...
#import "D5LedBoxOnOffRead.h"

@implementation D5LedBoxOnOffRead

- (void)ledBoxOnOffRead {
    @autoreleasepool {
        LedHeader *header = [self headerForCmd:Cmd_Box_Operate withSubCmd:SubCmd_Box_OnOff_Read];
...
- (void)getResult:(LedHeader *)header body:(NSDictionary *)body from:(NSString *)ip{
    @autoreleasepool {
        if (header->cmd != Cmd_Box_Operate || header->subCmd != SubCmd_Box_OnOff_Read) {
            return;
        }
        ...
        [[D5LedModule sharedLedModule].resultMuticast.delegateList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                if (obj && [obj respondsToSelector:@selector(ledBoxOnOffReadReturn:)]) {
                    [obj ledBoxOnOffReadReturn:status];
                }
            }
        }];
    }
