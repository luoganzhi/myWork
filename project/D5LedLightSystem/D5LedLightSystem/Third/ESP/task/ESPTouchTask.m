...
- (void) __putEsptouchResultIsSuc: (BOOL) isSuc AndBssid: (NSString *)bssid AndInetAddr:(NSData *)inetAddr
{
    ...
    if (DEBUG_ON)
    {
        //DLog(@"ESPTouchTask __putEsptouchResult(): count = %d",count);
    }
    ...
    if (!isTaskSucCountEnough)
    {
        if (DEBUG_ON)
        {
            //DLog(@"ESPTouchTask __putEsptouchResult(): count = %d, isn't enough", count);
        }
    ...
    if (!isExist)
    {
        if (DEBUG_ON)
        {
            //DLog(@"ESPTouchTask __putEsptouchResult(): put one more result");
        }
...
- (void) beginBackgroundTask
{
    if (DEBUG_ON)
    {
        //DLog(@"ESPTouchTask beginBackgroundTask() entrance");
    }
    self._backgroundTask = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
        if (DEBUG_ON)
        {
            //DLog(@"ESPTouchTask beginBackgroundTask() endBackgroundTask");
        }
...
- (void) endBackgroundTask
{
    if (DEBUG_ON)
    {
        //DLog(@"ESPTouchTask endBackgroundTask() entrance");
    }
...
- (void) __listenAsyn: (const int) expectDataLen
{
    ...
    dispatch_async(queue, ^{
        ...
        if (DEBUG_ON)
        {
            //DLog(@"ESPTouchTask __listenAsyn() start an asyn listen task, current thread is: %@", [NSThread currentThread]);
        }
        ...
        if (DEBUG_ON)
        {
            //DLog(@"ESPTouchTask __listenAsyn() expectOneByte: %d",expectOneByte);
        }
        ...
        while ([self._esptouchResultArray count] < [self._parameter getExpectTaskResultCount] && !self._isInterrupt)
        {
            ...
            if (receiveOneByte == expectOneByte)
            {
                if (DEBUG_ON)
                {
                    //DLog(@"ESPTouchTask __listenAsyn() receive correct broadcast");
                }
                ...
                if (timeout < 0)
                {
                    if (DEBUG_ON)
                    {
                        //DLog(@"ESPTouchTask __listenAsyn() esptouch timeout");
                    }
                    break;
                }
                else
                {
                    if (DEBUG_ON)
                    {
                        //DLog(@"ESPTouchTask __listenAsyn() socketServer's new timeout is %d milliseconds",timeout);
                    }
            ...
            else
            {
                if (DEBUG_ON)
                {
                    //DLog(@"ESPTouchTask __listenAsyn() receive rubbish message, just ignore");
                }
            }
        }
        ...
        if (DEBUG_ON)
        {
            //DLog(@"ESPTouchTask __listenAsyn() finish");
        }
        [self endBackgroundTask];
    });
}
...
- (BOOL) __execute: (ESPTouchGenerator *)generator
{
    ...
    while (!self._isInterrupt)
    {
        if (currentTime - lastTime >= [self._parameter getTimeoutTotalCodeMillisecond]/1000.0)
        {
            if (DEBUG_ON)
            {
                //DLog(@"ESPTouchTask __execute() send gc code ");
            }
            // send guide code
...
- (NSArray*) executeForResults:(int) expectTaskResultCount
{
    ...
    if (DEBUG_ON)
    {
        //DLog(@"ESPTouchTask executeForResult() localInetAddr: %@", [ESP_NetUtil descriptionInetAddrByData:localInetAddrData]);
    }
...
- (BOOL) __sleep :(long) milliseconds
{
    if (DEBUG_ON)
    {
        //DLog(@"ESPTouchTask __sleep() start");
    }
    ...
    if (DEBUG_ON)
    {
        //DLog(@"ESPTouchTask __sleep() end, receive signal is %@", signaled ? @"YES" : @"NO");
    }
    return signaled;
}
