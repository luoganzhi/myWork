...
- (void)pingFoundation:(PingFoundation *)pinger didStartWithAddress:(NSData *)address
{
    ...
    NSLog(@"start时的time = %f", time);
    [[NSUserDefaults standardUserDefaults] setDouble:time forKey:@"start_check_time"];
...
- (void)pingFoundation:(PingFoundation *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber {
    double time = [[NSUserDefaults standardUserDefaults] doubleForKey:@"start_check_time"];
    NSTimeInterval currentTime = [[NSDate date] timeIntervalSince1970];
    
    NSLog(@"receive时的time = %f, %f, %f", time, currentTime, currentTime - time);
    if (time != 0) {
