...
- (void)operateAllAddedLight:(LedOperateType)operateType parameterDict:(NSDictionary *)dict {
    @autoreleasepool {
        ...
        [sendDict addEntriesFromDictionary:dict]; //dict为nil也可以作为此方法的参数，不会报错
        
        D5LedNormalCmd *cmd = [[D5LedNormalCmd alloc] init];
        
        cmd.remoteLocalTag = tag_remote;
        cmd.remotePort = [D5CurrentBox currentBoxTCPPort];
        cmd.strDestMac = [D5CurrentBox currentBoxMac];
        cmd.remoteIp = [D5CurrentBox currentBoxIP];
        cmd.errorDelegate = self;
        cmd.receiveDelegate = self;
        
        [cmd ledSendData:Cmd_Led_Operate withSubCmd:SubCmd_Led_Operate_All withData:sendDict];
    }
}

- (void)operateModeSetting:(LedSceneType)sceneType bSame:(BOOL)bSame colorArr:(NSArray *)arr {
    @autoreleasepool {
        NSDictionary *sendDict = @{LED_STR_TYPE : @(sceneType),
                                   LED_STR_BSAME: @(bSame),
                                   LED_STR_LIST : arr};
        ...
        [ledSceneNew ledSendData:Cmd_Led_Operate withSubCmd:SubCmd_Led_Scene withData:sendDict];
    }
}
...
- (void)ledCmdReceivedData:(LedHeader *)header withData:(NSDictionary *)dict {
    @autoreleasepool {
        ...
        [self handleReceivedData:data header:header isFinished:YES errorType:-1 errorMessage:nil];
    }
}
 ...
 */
- (void)handleAddOKCmdReturn:(BOOL)isSuccess errorType:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage {
    if ([self checkDelegate]) {
        if ([_delegate respondsToSelector:@selector(ledListAddOK:isFinished:)]) {
            [_delegate ledListAddOK:self isFinished:isSuccess];
        }
        
        if (!isSuccess && [_delegate respondsToSelector:@selector(ledListAddOKError:errorType:errorMessage:)]) {
            [_delegate ledListAddOKError:self errorType:errorType errorMessage:errorMesssage];
        }
    }
}
 ...
 */
- (void)handleOperateNewCmd:(LedOperateNewType)type isFinished:(BOOL)isFinished errorType:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage {
    if ([self checkDelegate]) {
        if ([_delegate respondsToSelector:@selector(ledListOperateNewLed:operateType:isFinished:)]) {
            [_delegate ledListOperateNewLed:self operateType:type isFinished:isFinished];
        }
        
        if (!isFinished && [_delegate respondsToSelector:@selector(ledListErrorOperateNewLed:operateType:errorType:errorMessage:)]) {
            [_delegate ledListErrorOperateNewLed:self operateType:type errorType:errorType errorMessage:errorMesssage];
        }
    }
}
...
/**
 ...
 @param isFinished 是否操作成功
 */
- (void)handleOperateSingleAddedCmd:(LedOperateType)type isFinished:(BOOL)isFinished errorType:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage {
    if ([self checkDelegate]) {
        if ([_delegate respondsToSelector:@selector(ledListOperateAddedLed:operateType:isFinished:)]) {
            [_delegate ledListOperateAddedLed:self operateType:type isFinished:isFinished];
        }
        
        if (!isFinished && [_delegate respondsToSelector:@selector(ledListErrorOperateAddedLed:operateType:errorType:errorMessage:)]) {
            [_delegate ledListErrorOperateAddedLed:self operateType:type errorType:errorType errorMessage:errorMesssage];
        }
    }
}
...
/**
 ...
 @param isFinished 是否操作成功
 */
- (void)handleOperateAllAddedCmd:(LedOperateType)type isFinished:(BOOL)isFinished errorType:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage {
    if ([self checkDelegate]) {
        if ([_delegate respondsToSelector:@selector(ledListOperateAllLed:operateType:isFinished:)]) {
            [_delegate ledListOperateAllLed:self operateType:type isFinished:isFinished];
        }
        
        if (!isFinished && [_delegate respondsToSelector:@selector(ledListErrorOperateAllLed:operateType:errorType:errorMessage:)]) {
            [_delegate ledListErrorOperateAllLed:self operateType:type errorType:errorType errorMessage:errorMesssage];
        }
    }
}
...
场景设置的结果
 ...
 @param isFinished 是否操作成功
 */
- (void)handleOperateMode:(LedSceneType)type isFinished:(BOOL)isFinished errorType:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage {
    if ([self checkDelegate]) {
        if ([_delegate respondsToSelector:@selector(ledListOperateMode:sceneType:isFinished:)]) {
            [_delegate ledListOperateMode:self sceneType:type isFinished:isFinished];
        }
        
        if (!isFinished && [_delegate respondsToSelector:@selector(ledListErrorOperateMode:sceneType:errorType:errorMessage:)]) {
            [_delegate ledListErrorOperateMode:self sceneType:type errorType:errorType errorMessage:errorMesssage];
        }
    }
}
...
- (void)d5NetWorkError:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage data:(NSDictionary *)data forHeader:(LedHeader *)header {
    [self handleReceivedData:data header:header isFinished:NO errorType:errorType errorMessage:errorMesssage];
}

- (void)handleReceivedData:(NSDictionary *)data header:(LedHeader *)header isFinished:(BOOL)isFinished errorType:(D5SocketErrorCodeType)errorType errorMessage:(NSString *)errorMesssage {
    if (header->cmd == Cmd_Led_Operate) {
        switch (header->subCmd) {
            ...
            case SubCmd_Led_Add_Ok:
                [self handleAddOKCmdReturn:isFinished errorType:errorType errorMessage:errorMesssage];
                break;
                
            case SubCmd_Led_Operate_New: {
                LedOperateNewType type = -1;
                if (data) {
                    type = [data[LED_STR_TYPE] intValue];
                }
                [self handleOperateNewCmd:type isFinished:isFinished errorType:errorType errorMessage:errorMesssage];
            }
                break;
                
            case SubCmd_Led_Operate_Single: {
                LedOperateType type = -1;
                if (data) {
                    type = [data[LED_STR_TYPE] intValue];
                }
                [self handleOperateSingleAddedCmd:type isFinished:isFinished errorType:errorType errorMessage:errorMesssage];
            }
                break;
                
            case SubCmd_Led_Operate_All: {
                LedOperateType type = -1;
                if (data) {
                    type = [data[LED_STR_TYPE] intValue];
                }
                [self handleOperateAllAddedCmd:type isFinished:isFinished errorType:errorType errorMessage:errorMesssage];
            }
                break;
                
            case SubCmd_Led_Scene: {
                LedSceneType type = -1;
                if (data) {
                    type = [data[LED_STR_TYPE] intValue];
                }
                [self handleOperateMode:type isFinished:isFinished errorType:errorType errorMessage:errorMesssage];
            }
                break;
            default:
                break;
