...
- (void)initView {
    ...
    _bgImage = [UIImage imageNamed:@"colour_turnon"];
...
- (D5WheelItem *)baseWheelItem {
    if (!_baseWheelItem) {
        ...
        baseWheelItemFrame.origin.y = CGRectGetMinY(self.frame); //如果是上面点，则为minY，如果下面点，则为midY
        ...
        if (gesture.state == UIGestureRecognizerStateChanged) {//rotate
            ...
            [UIView animateWithDuration:0.3f animations:^{
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(wheelView:didSelectedItemAtIndex:)]) {
                    [self.delegate wheelView:self didSelectedItemAtIndex:gesture.selectedIndex];
                }
            }];
        }
    }
