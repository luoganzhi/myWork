...
#import "D5LedBrightnessAll.h"

@implementation D5LedBrightnessAll

- (void)ledBrightnessAll:(uint8_t)brightness {
    @autoreleasepool {
        LedHeader *header = [self headerForCmd:Cmd_Led_Operate withSubCmd:SubCmd_Led_Brightness_All];
        ...
        [data appendBytes:&brightness length:sizeof(uint8_t)];//+
...
- (void)getResult:(LedHeader *)header body:(NSDictionary *)body from:(NSString *)ip{
    @autoreleasepool {
        if (header->cmd != Cmd_Led_Operate || header->subCmd != SubCmd_Led_Brightness_All) {
            return;
        }
        ...
        [[D5LedModule sharedLedModule].resultMuticast.delegateList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                if (obj && [obj respondsToSelector:@selector(ledBrightnessAllReturn:)]) {
                    [obj ledBrightnessAllReturn:status];
                }
            }
        }];
    }
