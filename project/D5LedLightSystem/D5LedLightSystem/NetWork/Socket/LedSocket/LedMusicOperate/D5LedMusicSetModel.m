...
#import "D5LedMusicSetModel.h"

@implementation D5LedMusicSetModel

- (void)ledMusicSetModel:(LedMusicSetModelType)type
{
    @autoreleasepool {
        LedHeader *header = [self headerForCmd:Cmd_Music_Operate withSubCmd:SubCmd_Music_Set_Model];
...
- (void)getResult:(LedHeader *)header body:(NSDictionary *)body from:(NSString *)ip{
    @autoreleasepool {
        if (header->cmd != Cmd_Music_Operate || header->subCmd != SubCmd_Music_Set_Model) {
            return;
        }
        ...
        [[D5LedModule sharedLedModule].resultMuticast.delegateList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            @autoreleasepool {
                if (obj && [obj respondsToSelector:@selector(ledMusicSetModelReturn:)]) {
                    [obj ledMusicSetModelReturn:status];
                }
            }
        }];
    }
