...
@property (strong, nonatomic) GCDAsyncSocket * tcpSockt;
@property (strong, nonatomic) GCDAsyncUdpSocket * udpSockt;
...
-(void)udpSocket:(GCDAsyncUdpSocket *)sock didSendDataWithTag:(long)tag{
    //DLog(@"send udp data success:%ld",tag);
}
-(void)udpSocketDidClose:(GCDAsyncUdpSocket *)sock withError:(NSError *)error{
    //DLog(@"udp closed :%@",error.localizedDescription);
    [self initialUdpConnect];
}
-(void)udpSocket:(GCDAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError *)error{
    //DLog(@"udp send error:%@",error.localizedDescription);
...
-(void)udpSocket:(GCDAsyncUdpSocket *)sock didNotConnect:(NSError *)error{
    //DLog(@"udp connect error:%@",error.localizedDescription);
    [self initialUdpConnect];
}
...
-(void)socket:(GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port{
    //DLog(@"tcp connected to %@ successfull",host);
...
//    //DLog(@"tcp send data successfull");
}
